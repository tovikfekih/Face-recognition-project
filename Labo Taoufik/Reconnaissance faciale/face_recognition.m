...
% test='C:\Users\extra\OneDrive\Etudes\Projet Tut - Reconnaissance Faciale\BDD images\TEST 2\345.jpg';



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[baseName, folder]= uigetfile('*.jpg');
test=strcat(folder,baseName);
test='C:\Users\extra\OneDrive\Etudes\Projet Tut - Reconnaissance Faciale\BDD images\BDD 3\male\9338446\9338446.15.jpg';
imsa=extraireVisage(test,0,0)

learn = getAllFiles(datapath);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%  création de la matrice image X  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if(exist('X.mat')==2)
    % Comme la maudite matrice X prends du temps pour sa création, nous
    % avons choisis de la conserver et la récupérer si elle existe déja,
    % sinon on va la créer.
    disp('la matrice image X existe, on va la charger');
    load('X.mat','X');
else
    disp('La matrice image X n''existe pas, on va la créer');
    X = [];
    t=size(imread(learn{1}),1)
    for i = 1 : imgcount
        disp(strcat('Image N° ',int2str(i),' sur ',int2str(imgcount),' a été chargé dans la matrice image X'));
        img=imread(learn{i});
%         img = rgb2gray(img);
     ...
     disp('Matrice image X sauvegardé !');
     save('X.mat','X');
    end
...
Y= A' * A;
[V,D]=eig(Y);  %% V : eigenvector matrix  D : eigenvalue matrix
...
montage(learn(distances_index(1:10)))

% [euclide_dist_min recognized_index] = min(euclide_dist);
% recognized_img=learn{recognized_index};
% figure;imshow(learn{recognized_index});
